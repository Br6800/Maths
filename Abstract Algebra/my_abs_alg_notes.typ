...
#let gen(x) = math.op($angle.l #x angle.r$)
...
= Subgroups
Subgroups are closed, contain identity and inverses. 
An element $a$ generates $G$ if
the cyclic subgroup $gen(a) = { a^n : n in ZZ } = G$. We say that $G$ 
is cyclic if there is some element that generates it.
So obviously $gen(a)$ is a cyclic group (its always a group). 
Any group with no proper nontrivial subgroups is generated by every
non identity element (except the trivial group (or not, vacuous??)), so is cyclic.
= Cyclic Groups
Subgroup of a cyclic group is cyclic.
#align(center)[
  #image("fig/IMG_7322.jpeg",width:70%)
  ]
Pick any subgroup $H$ of the integers under addition. 
Then $H$ is cyclic, so some $h$ generates $H$. So $h in ZZ$
 and we must have $H = h ZZ$. The following is unrelated. 
 A finicky but easy enough proof shows that any cyclic group is isomorphic 
 to the integers under addition or the integers under modular addition.
The prior fact about subgroups of $ZZ$ characterises subgroups of infinite cyclic groups, since their parents are isomorphic to $ZZ$.
...
#theorem()[
Let $G = gen(a)$ be a cyclic group of order $n$. Then the order of the subgroup generated by
$a^m$ is $n / d$. Further, $gen(a^m) = gen(a^p)$ iff $gcd(m,n) = gcd(p,n)$.
#proof()[
Go read the first bit. The iff statement is badly explained. What they are saying is that
$
  {m in ZZ_n:gcd(m,n) = d} subset gen(d)
$
since $d | m implies m = k d = d^k in gen(d)$. Any subgroup
with $n / d$ elements contains all elements $a^p$ of $G$ such that
$gcd(p,n) = d$. In the case of $ZZ_n$, this is just integers $p$ with $gcd(p,n) = d$.
Then $gen(a^m)$ contains $a^p$ and $gen(a^p) subset gen(a^m)$ and similarly $gen(a^m) subset gen(a^p)$ so $gen(a^m) = gen(a^p)$. That is to say, if the gcd's are the same then the subgroups are the same.
If the subgroups are the same, they have the same number of elements, $n / d_1 = n / d_2 implies d_1 = d_2$.
]
]
#cor()[
...
$
  {k in ZZ:m | k, n | k}.
$
The smallest possible LCM is $m n$ because if $k | m$ then $m n = (a k) n = (a n) k$ so $k | m n$. If $m,n$ are coprime then $m | n k$ implies $m | k$. In particular, if $n | k$ then $k = q n$ so $m | k$ implies $m | q$ so that $k = q n = (a m) n = a (m n)$, i.e. $m n | k$.
]
#theorem()[
...
$
#proof()[
Let $lcm(m,n) = a m = b n$. Then
$
  (m n) / lcm(m,n) * a = n,
  (m n) / lcm(m,n) * b = m
$ so it is a divisor of $a,b$, proving $(m n) / gcd(m,n) <= lcm(m,n).$
Since $(m n) / gcd(m,n)$ is a multiple of both $m$ and $n$, $(m n) / gcd(m,n) >= lcm(m,n)$.
]]
#theorem()[
Abelian groups with cyclic subgroups $H,K$ 
of coprime orders $r$ and $s$ have a cyclic subgroup of order $r s$. 
More generally, there is a subgroup of order $lcm(r,s)$.
#proof()[
We know that $H cap K$ is a cyclic subgroup. So it must be generated by $x = h^p  = k^q$.
Let $m = abs(gen(x))$. Then $m = r / gcd(p,r) = s / gcd(q,s)$ so $m | r$ and $m | s$. Then $m | gcd(r,s) = 1$ and $m = 1$. So $H cap K$ is the trivial group. Now consider the group $Z = {x y:x in H, y in K  }$ (it is a group since the whole group is Abelian).
Since $H,K$ are finite cyclic groups,  there generators $h,k$ of $H,K$.
We define a bijection $f:Z_r times Z_s to Z$, $f(a,b) = h^a k^b$. if $f(a,b) = f(c,d)$, 
then $h^(a-b) = k^(d-c) = e$ since $H cap K  = {e}$. So $(a,b) = (c,d)$. 
Fix $z in Z$. Then $z = x y = h^a k^b$ where $0 <= a,b < r,s$ so $f(a,b) = z$. So $f$ is a bijection as asserted. There are $r s$ elements in $Z_r times Z_s$ so the cardinality of $Z$ is $r s$.
We claim that $Z$ is cyclic when $r,s$ are coprime and $g h$ generates $Z$.
Since $r,s$ are coprime there exist integers $m_1,m_2$ so that $1 = m_1 r + m_2 s$. Fix $z in Z$ and write $z = h^a k^b$. Then we have
$
  (h k)^(b m_1 r + a m_2 s) = h^(b m_1 r + a m_2 s) * k^(b m_1 r + a m_2 s) = 
  h^(a m_2 s) * k^(b m_1 r) = h^(a(1 - m_1 r)) * k^(b(1 - m_2 s)) = 
  h^a * k^b = z.
$
N.B. The Chinese remainder theorem applies here.
]
#proof()[
Easier proof. Consider $gen(h k)$. This group is cyclic since it is generated by $h k$.
The order is no more than $r s$, since $(h k)^{r s} = e^s * e^r = e$. Suppose $p < r s$ was the order of $gen(h k)$. 
Then $h^p = k^(-p)$ so that $h^p in H cap K$. So $gen(h^p) = d | r,s$ which implies $d = 1$, $gen(h^p) = { e }$. It follows that $h^p = k^p = e$, so $r,s | p$ (because they are the orders of $H,K$). Then $p = a r$, so $s | a$ and $p = b r s$, $r s | p$ which gives a contradiction, $p >= r s$.
]
] <theorem7.1.3>
#lemma()[
Given $r,s$ we can construct coprime $a,b$ so that $a | r$, $b | s$ and
$a b = lcm(r,s)$.
#proof()[
Write $lcm(r,s) = d * (r / d) * (s / d)$ and $
d = p_1^(a_1) ... p_n^(a_n) * q_1^(b_1) ... q_m^(b_m)
$ where $p_i divides.not r / d$ and $q_i | r / d$. Then $q_i divides.not s / d$. Consider $
a = p_1^(a_1) ... p_n^(a_n) * r / d, \
b = q_1^(b_1) ... q_m^(b_m) * s / d.
$N.B. if $a b | m$ then $a | m$, $b | m$ contrapositive is if neither divide $m$ then their product doesn't.
]
]
#proof()[(of the latter part of @theorem7.1.3): #linebreak()
By the lemma, we can construct coprime divisors $a,b$ of $r,s$ with $a b = lcm(r,s)$ so that for $r = a del_1$, $s = b del_2$ and $gen(h^{del_1}) <= gen(h)$, $gen(k^{del_2}) <= gen(k)$ we have
$
  abs(gen(h^{del_1})) = r / gcd(del_1,r) = r / del_1 = a, \
  abs(gen(k^{del_2})) = s / gcd(del_2,s) = s / del_2 = b.
  $
The first part of the theorem completes the proof.
]
#ex()[
There is a generator of $Z_{36}$ because it has subgroups $gen(2)$, $gen(3)$ with $12$ and $18$ elements. We write $12 = 6 * 2$ and $18 = 6 * 3$, then separate factors to get $6 = 3 * 2$, $a = 2 * 12 / 6 = 4$, $b = 3 * 18 / 6 = 9$ so that $12 = 4 * 3$ and $18 = 9 * 2$ so we can take these new factors $3,2$ and raise existing generators to them. We then have $gen(3^3) = gen(9)$ of order $4$ and $gen(2^2) = gen(4)$ of order $9$. So we can generate $Z_{36}$ with $9+4 = 13$.
]
== Cayleigh Digraphs
#defn()[An arc in a cayleigh digraph is a directed edge that represents multiplication by a generator. When a generator is an involution we use undirected arcs.]
An obvious implication of this is that in a group, if $a,b,...,x,y$ are involutions then
$
  (a b ... x y)^(-1) = y^(-1) x^(-1) ... b^(-1) a^(-1) = y x ... b a.
$
#defn()[A vertex in a cayleigh digraph is an element generated by the generators of the graph.]
